  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
      ...
      <div
        ...
        className={`relative z-10 w-full ${modalWidth} rounded-3xl bg-white dark:bg-[#0A0A0A] border border-slate-200 dark:border-white/[0.1] ${
          noPadding ? "p-0 overflow-hidden" : "p-6 overflow-y-auto"
        } max-h-[90vh] shadow-2xl animate-scaleIn my-auto text-slate-900 dark:text-slate-100 transition-colors`}
      >
        {noPadding ? (
          <>
            {onClose && (
              <button
                ...
                className="absolute top-4 right-4 z-20 shrink-0 h-8 w-8 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-100 dark:hover:bg-white/[0.08] hover:text-slate-900 dark:hover:text-white transition-colors bg-white/60 dark:bg-black/40 backdrop-blur-sm border border-slate-200/60 dark:border-white/[0.08]"
              >
