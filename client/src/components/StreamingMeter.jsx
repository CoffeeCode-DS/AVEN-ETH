...
export default function StreamingMeter({
  ...
  return (
    <div className="p-6 rounded-2xl bg-gradient-to-br from-[#141414] to-[#0A0A0A] border border-white/[0.08] text-white relative overflow-hidden shadow-2xl">
      <div className="absolute -top-24 -right-24 h-64 w-64 rounded-full bg-[#6366F1]/15 blur-3xl pointer-events-none" />
      ...
      <div className="relative flex items-center justify-between gap-4 flex-wrap mb-4 font-mono">
        ...
        <div className="text-xs text-slate-300 bg-white/[0.06] border border-white/[0.08] px-3 py-1 rounded-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-6 pt-2 font-mono">
        <div>
          <p className="text-xs text-slate-400 mb-1">Total Accrued (Earned)</p>
          <p className="text-2xl sm:text-3xl font-bold tracking-tight text-white">
            {formatEth(liveEarned)}
          </p>
          <p className="text-[11px] text-slate-500 mt-1">
            of <span className="text-slate-300 font-mono">{formatEth(budget)}</span> total deposit
          </p>
        </div>

        <div>
          <p className="text-xs text-slate-400 mb-1">Claimable Available</p>
          <p className="text-2xl sm:text-3xl font-bold tracking-tight text-emerald-400">
        ...
        <div>
          <p className="text-xs text-slate-400 mb-1">Already Withdrawn</p>
          <p className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-300">
      ...
      {isFreelancer && liveAvailable > 0.0001 && (
        <div className="mt-6 pt-5 border-t border-white/[0.06] flex items-center justify-between gap-4 flex-wrap">
          <p className="text-xs font-mono text-slate-300">
            You have <strong className="text-emerald-400">{formatEth(liveAvailable)}</strong> accrued that can be claimed on-chain now.
          </p>
          <button
            onClick={() => onClaim(liveAvailable)}
            disabled={claiming}
            className="h-9 px-4 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-mono text-xs font-semibold uppercase tracking-wide transition-all shadow-md shadow-emerald-500/20"
          >
